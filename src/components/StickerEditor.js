...
export default function StickerEditor({ photo, stickerUrl, onClose, onApplied }) {
  ...
  async function apply() {
    setApplying(true);
    try {
      // Render photo + sticker to canvas at natural photo size
      ...
      // The container fits the photo via object-fit:contain.
      // We need to map sticker position from container space to photo pixel space.
      ...
      // Sticker default base size = 30% of container width
  ...
  return (
    <div className="fixed inset-0 bg-black z-[100] flex flex-col">
      ...
      <div className="p-4 bg-zinc-900 flex justify-center gap-3">
        <button onClick={() => setRotation(r => r - 15)} className="px-3 py-2 bg-zinc-800 rounded">↺</button>
        <button onClick={() => setRotation(r => r + 15)} className="px-3 py-2 bg-zinc-800 rounded">↻</button>
        <button onClick={() => setScale(s => Math.max(0.2, s - 0.1))} className="px-3 py-2 bg-zinc-800 rounded">−</button>
        <button onClick={() => setScale(s => Math.min(4, s + 0.1))} className="px-3 py-2 bg-zinc-800 rounded">+</button>
      </div>
    </div>
  );
}
